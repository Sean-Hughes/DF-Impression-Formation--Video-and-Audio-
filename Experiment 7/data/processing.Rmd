...
genuine_data <- c("raw/Experiment_7_Genuine_data.zip")

unzip(genuine_data, exdir = "raw")

deepfaked_data <- c("raw/Experiment_7_DF_data.zip")
...
# Genuine video: Connect to the SQL Database
...
connection <- dbConnect(
  drv=RSQLite::SQLite(),
  dbname= "raw/Experiment_7_Genuine_data.sqlite")

# Extract main table
database_genuine_video <- dbGetQuery(
  conn=connection,
...
database_genuine_video_meta_data <- map_dfr(database_genuine_video$metadata, fromJSON) %>%
  dplyr::rename(observation = id)

database_genuine_video <- database_genuine_video %>%
  bind_cols(database_genuine_video_meta_data) %>%
  
  # Remove metadata column
  select(-metadata)

# Remove temporary data frame
rm(database_genuine_video_meta_data)
...
for (i in 5:36) {
  if (
    information_preserved(database_genuine_video$session, i) &&
    information_preserved(database_genuine_video$observation, i)
  ) {
    break()
  }
}
...
database_genuine_video_full <- database_genuine_video %>%
  dplyr::filter(payload == 'full') 

if (nrow(database_genuine_video_full) > 0) {
  database_genuine_video_full %>%
    ...
    ungroup() %>%
    select(-id) -> database_genuine_video_full
}
...
database_genuine_video %>%
  ...
  ungroup() %>%
  select(-id) -> database_genuine_video_incremental
...
raw_data_genuine_video <- database_genuine_video_full %>%
  bind_rows(
    database_genuine_video_incremental %>% 
...
raw_data_genuine_video <- raw_data_genuine_video %>% 
...
raw_data_genuine_video %>%
  group_by(subject) %>%
  fill(matches('code'), .direction='down') %>%
  fill(matches('code'), .direction='up') %>%
  ungroup() -> raw_data_genuine_video

rm(database_genuine_video, database_genuine_video_full, database_genuine_video_incremental)
...
Raw_Data_genuine <- raw_data_genuine_video 
```


#  Deepfaked video: Connect to the SQL Database
...
connection <- dbConnect(
  drv=RSQLite::SQLite(),
  dbname= "raw/Experiment_7_DF_data.sqlite")

# Extract main table
database_deepfaked_video <- dbGetQuery(
  conn=connection,
  statement='SELECT * FROM labjs')
...
database_deepfaked_video_meta_data <- map_dfr(database_deepfaked_video$metadata, fromJSON) %>%
  dplyr::rename(observation = id)

database_deepfaked_video <- database_deepfaked_video %>%
  bind_cols(database_deepfaked_video_meta_data) %>%
  
  # Remove metadata column
  select(-metadata)

# Remove temporary data frame
rm(database_deepfaked_video_meta_data)
...
for (i in 5:36) {
  if (
    information_preserved(database_deepfaked_video$session, i) &&
    information_preserved(database_deepfaked_video$observation, i)
  ) {
    break()
  }
}
...
database_deepfaked_video_full <- database_deepfaked_video %>%
  dplyr::filter(payload == 'full') 

if (nrow(database_deepfaked_video_full) > 0) {
  database_deepfaked_video_full %>%
    group_by(observation, id) %>%
    do(
      { map_dfr(.$data, parseJSON) } %>%
        bind_rows()
    ...
    ungroup() %>%
    select(-id) -> database_deepfaked_video_full
}
...
database_deepfaked_video %>%
  ...
  ungroup() %>%
  select(-id) -> database_deepfaked_video_incremental
...
raw_data_deepfaked_video <- database_deepfaked_video_full %>%
  bind_rows(
    database_deepfaked_video_incremental %>% 
      filter(!(observation %in% database_deepfaked_video_full$observation))) %>%
  type_convert()
...
raw_data_deepfaked_video <- raw_data_deepfaked_video %>% 
  rename(subject = observation)
...
raw_data_combined <- raw_data_genuine_video %>% 
  full_join(raw_data_deepfaked_video)
...
data_self_reports <- raw_data_combined %>% 
  ...
  mutate(behavioral_intentions = (intentions_q1 + intentions_q2 + intentions_q3)/3) %>%
