...
- prereg says we'll calculate POMP scores for all individual differences variables prior to analysis
- what else does the prereg say that we'll have to add?
...
data_exploratory_questions <- data_trial_level %>% 
  ...
  summarise_all(~ first(na.omit(.))) %>%
  ungroup() %>%
  mutate(diagnosticity_question = case_when(grepl("Not", diagnosticity_question) ~ 0,
                                            grepl("Slight", diagnosticity_question) ~ 1,
                                            grepl("Moderate", diagnosticity_question) ~ 2,
                                            grepl("Strong", diagnosticity_question) ~ 3),
         demand = case_when(grepl("No", demand) ~ 0,
                            grepl("I", demand) ~ 0,
                            grepl("Demand", demand) ~ 1),
         reactance = case_when(grepl("No", reactance) ~ 0,
                               grepl("I", reactance) ~ 0,
                               grepl("React", reactance) ~ 1),
         hypothesis_awareness = case_when(grepl("Yes", hypothesis_awareness) ~ 1,
                                          grepl("No", hypothesis_awareness) ~ 0),
         influence_awareness = case_when(grepl("Yes", influence_awareness) ~ 1,
                                         grepl("No", influence_awareness) ~ 0))
...
data_aot <- data_individual_differences_temp %>%
  ...
  summarize(aot_actively_openminded_thinking_sum = sum(aote_score, na.rm = TRUE),
            aot_n_items = n(),
            .groups = "drop") %>%
  mutate(aot_actively_openminded_thinking_sum = ifelse(aot_n_items == 8, aot_actively_openminded_thinking_sum, NA)) %>%
  select(-aot_n_items)
...
data_bcti <- data_individual_differences_temp %>%
  ...
  summarize(bcti_belief_in_conspiracy_sum = sum(as.numeric(bcti_score), na.rm = TRUE),
            bcti_n_items = n(),
            .groups = "drop") %>%
  mutate(bcti_belief_in_conspiracy_sum = ifelse(bcti_n_items == 15, bcti_belief_in_conspiracy_sum, NA)) %>%
  select(-bcti_n_items)
...
data_crt <- data_individual_differences_temp %>%
  ...
  summarize(crt_analytic_thinking_sum = sum(crt_score, na.rm = TRUE),
            crt_n_items = n(),
            .groups = "drop") %>%
  mutate(crt_analytic_thinking_sum = ifelse(crt_n_items == 7, crt_analytic_thinking_sum, NA)) %>%
  select(-crt_n_items)
...
data_ocq <- data_individual_differences_temp %>%
  ...
  summarize(ocq_overclaiming_sum = sum(ocq_score),
            ocq_n_items = n(),
            .groups = "drop") %>%
  mutate(ocq_overclaiming_sum = ifelse(ocq_n_items == 30, ocq_overclaiming_sum, NA)) %>%
  select(-ocq_n_items)
...
data_ras <- data_individual_differences_temp %>%
  ...
  summarize(ras_relgious_affliation_sum = sum(ras_score, na.rm = TRUE),
            ras_n_items = n(),
            .groups = "drop") %>%
  mutate(ras_relgious_affliation_sum = ifelse(ras_n_items == 8, ras_relgious_affliation_sum, NA)) %>%
  select(-ras_n_items)
...
data_rei <- data_individual_differences_temp %>%
  ...
  mutate(rei_score = ifelse(rei_item %in% c("q1_rei", "q4_rei", "q6_rei", "q7_rei", "q10_rei", 
                                            "q13_rei", "q15_rei", "q18_rei", "q19_rei", "q20_rei"), 8-as.numeric(rei_score), as.numeric(rei_score)),
         rei_subscale = ifelse(rei_item %in% c("q1_rei", "q2_rei", "q6_rei", "q7_rei", "q10_rei", 
                                           "q11_rei", "q13_rei", "q15_rei", "q19_rei", "q20_rei"), "rei_rational_scale", "rei_experiential_scale")) %>%
  ...
  summarize(rei_rational_sum = sum(rei_rational_scale, na.rm = TRUE),
            ...
            rei_n_items = n(),
            .groups = "drop") %>%
  mutate(rei_rational_sum = ifelse(rei_n_items == 20, rei_rational_sum, NA),
         rei_experiential_sum = ifelse(rei_n_items == 20, rei_experiential_sum, NA)) %>%
  select(-rei_n_items)
...
data_me_awareness <- data_individual_differences_temp %>%
  ...
  summarize(me_fake_news_awareness_sum = sum(me_fake_news, na.rm = TRUE),
            ...
            me_n_items = n(),
            .groups = "drop") %>%
  mutate(me_fake_news_awareness_sum = ifelse(me_n_items == 12, me_fake_news_awareness_sum, NA),
         me_real_news_awareness_sum = ifelse(me_n_items == 12, me_real_news_awareness_sum, NA)) %>%
  select(-me_n_items)
...
data_me_accuracy <- data_individual_differences_temp %>%
  ...
  summarize(me_fake_news_accuracy_sum = sum(as.numeric(me_fake_news), na.rm = TRUE),
            ...
            me_n_items = n(),
            .groups = "drop") %>%
  mutate(me_fake_news_accuracy_sum = ifelse(me_n_items == 12, me_fake_news_accuracy_sum, NA),
         me_real_news_accuracy_sum = ifelse(me_n_items == 12, me_real_news_accuracy_sum, NA)) %>%
  select(-me_n_items)
...
data_me_sharing <- data_individual_differences_temp %>%
  ...
  summarize(me_fake_news_sharing_sum = sum(me_fake_news, na.rm = TRUE),
            ...
            me_n_items = n(),
            .groups = "drop") %>%
  mutate(me_fake_news_sharing_sum = ifelse(me_n_items == 12, me_fake_news_sharing_sum, NA),
         me_real_news_sharing_sum = ifelse(me_n_items == 12, me_real_news_sharing_sum, NA)) %>%
  select(-me_n_items)
...
data_participant_level <- data_conditions_and_demographics %>% 
  ...
  full_join(data_me_awareness, by = "subject") %>%
...
# all participant level data
write_csv(data_participant_level, "processed/3_data_participant_level.csv")

# # just the open ended memory responses for hand scoring
# data_participant_level %>%
#   select(subject, memory_of_video_content) %>%
#   write_csv("processed/4_data_participant_level_detection_responses.csv")
