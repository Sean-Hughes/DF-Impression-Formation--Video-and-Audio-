...
data_trial_level <- data_preprocessed %>%
  ...
  mutate(task = tolower(task),
         # tidy up the various naming conventions between studies
         task = case_when(task == "video_positive_baseline"  ~ "video_positive_genuine",
                          task == "video_negative_baseline"  ~ "video_negative_genuine",
                          task == "video_positive_genuine"   ~ "video_positive_genuine",
                          task == "video_negative_genuine"   ~ "video_negative_genuine",
                          task == "video_positive_genuine"   ~ "video_positive_genuine",
                          task == "video_negative_genuine"   ~ "video_negative_genuine",
                          task == "video_positive_df"        ~ "video_positive_deepfaked",
                          task == "video_negative_df"        ~ "video_negative_deepfaked",
                          task == "video_positive_deepfaked" ~ "video_positive_deepfaked",
                          task == "video_negative_deepfaked" ~ "video_negative_deepfaked",
                          task == "audio_positive_baseline"  ~ "audio_positive_genuine",
                          task == "audio_negative_baseline"  ~ "audio_negative_genuine",
                          task == "audio_positive_genuine"   ~ "audio_positive_genuine",
                          task == "audio_negative_genuine"   ~ "audio_negative_genuine",
                          task == "audio_positive_deepfaked" ~ "audio_positive_deepfaked",
                          task == "audio_negative_deepfaked" ~ "audio_negative_deepfaked",
                          task == "audio_positive_deepfake"  ~ "audio_positive_deepfaked",
                          task == "audio_negative_deepfake"  ~ "audio_negative_deepfaked",
                          TRUE ~ task),
         experiment_condition_alt = case_when(task %in% c("video_positive_deepfaked",
                                                          "video_negative_deepfaked",
                                                          "audio_positive_deepfaked",
                                                          "audio_negative_deepfaked") ~ "deepfaked",
                                              task %in% c("video_positive_genuine",
                                                          "video_negative_genuine",
                                                          "audio_positive_genuine",
                                                          "audio_negative_genuine") ~ "genuine"),
         task = case_when(task == "stimulus" ~ "iat",
                          ...
                          # trials representing the intervention (audio/video)
                          task %in% c("video_positive_genuine",
                                      "video_negative_genuine",
                                      "video_positive_deepfaked",
                                      "video_negative_deepfaked",
                                      "audio_positive_genuine",
                                      "audio_negative_genuine",
                                      "audio_positive_deepfaked",
                                      "audio_negative_deepfaked") ~ "intervention",
                          TRUE ~ task)) %>%
  
  # propagate exp condition up and down rows within each participant
  group_by(subject) %>%
  fill(matches("experiment_condition_alt"), .direction = "down") %>%
  fill(matches("experiment_condition_alt"), .direction = "up") %>%
  ungroup() %>%
  ...
  # wrangle ratings
  mutate(ratings_item = case_when(task == "ratings_good_bad"          ~ "ratings_bad_good",
                                  task == "ratings_like_dislike"      ~ "ratings_dislike_like",
                                  task == "ratings_negative_positive" ~ "ratings_negative_positive"),
  ...
  select(subject,
         ...
         iat_stimulus,
         correct,
         latency, ## also used for intervention viewing time (task == "intervention")
         blockcode,
         praccrit,
...
write_rds(data_trial_level, "processed/2.1_data_trial_level.rds", compress = "gz")
# data_trial_level <- read_rds("processed/2.1_data_trial_level.rds")
...
data_conditions_and_demographics <- data_trial_level %>% 
  ...
  mutate(education_recoded = 
         ...
         # employment_recoded was mentioned in prereg but the groupings dont make a huge amount of sense to me, so I haven't included in processing.
         # employment_recoded = 
...
## Intervention (video/audio)

### Viewing time

```{r}

data_induction_linger_time <- data_trial_level %>%
  filter(task == "intervention") %>%
  select(subject, 
         experiment,
         latency) %>%
  mutate(intervention_linger_minutes = round(as.numeric(latency)/(1000*60), 3),
         exclude_implausible_intervention_linger = ifelse((experiment != 7 & intervention_linger_minutes <= 1.5) |
                                                            (experiment == 7 & intervention_linger_minutes <= 2.25) |
                                                            intervention_linger_minutes >= 4.5, 
                                                          TRUE, FALSE)) %>%
  drop_na()

```

### Open ended description of video/audio
...
data_induction_video <- data_trial_level %>% 
  select(subject, 
         memory_of_intervention_content = memory_of_video_content) %>% 
  drop_na()

data_induction_audio <- data_trial_level %>% 
  select(subject, 
         memory_of_intervention_content = memory_of_audio_content) %>% 
  drop_na()

data_induction_description <- bind_rows(data_induction_video, data_induction_audio)
...
data_completeness_iat <- data_trial_level_iat %>%
  count(experiment, subject) %>%
  # the number of trials in the IAT changed between experiments
  mutate(complete_iat = case_when(experiment %in% c("1", "2", "3", "4", "5", "6") & n == 128 ~ "complete", 
                                  experiment %in% c("1", "2", "3", "4", "5", "6") & n > 128 ~ "excess", 
                                  experiment %in% c("1", "2", "3", "4", "5", "6") & n < 128 ~"partial",
                                  experiment == "7" & n == 120 ~ "complete", 
                                  experiment == "7" & n > 120 ~ "excess", 
                                  experiment == "7" & n < 120 ~"partial")) %>%
  ungroup() %>%
  select(-n, -experiment)
...
data_iat_D2_scores <- data_iat_item_level %>%
  ...
  rename(subject = subject_original,
         IAT_D2 = p2112) %>%
  mutate(IAT_D2 = ifelse(abs(IAT_D2) > 2, NA, round(IAT_D2, 3))) # if D2 score is beyond theoretical limits, drop it. Otherwise round.
...
data_completeness_self_reported_evaluations <- data_trial_level_self_reported_evaluations %>%
  count(subject) %>%
  mutate(complete_selfreport = case_when(n == 3 ~ "complete", 
                                         n > 3 ~ "excess", 
                                         n < 3 ~ "partial")) %>%
  select(-n)
...
data_self_reported_evaluations_item_level <- data_trial_level_self_reported_evaluations %>%
  pivot_wider(names_from = ratings_item, 
              values_from = ratings_response,
              values_fn = {mean}) 
...
Mean scoring, completeness, and item level data

```{r}

data_behavioural_intentions_item_level <- data_trial_level %>%
  select(subject, 
         experiment,
         behavioral_intentions_share     = intentions_q1, 
         behavioral_intentions_subscribe = intentions_q2, 
         behavioral_intentions_recommend = intentions_q3) %>%
  drop_na() 

data_behavioural_intentions <- data_behavioural_intentions_item_level %>%  
  gather(intentions_item, intentions_response, c("behavioral_intentions_share", 
                                                 "behavioral_intentions_subscribe", 
                                                 "behavioral_intentions_recommend")) %>%
  # rescore from 1-to-5 to -2-to-+2 to make inversion easier
  mutate(intentions_response = case_when(experiment == 6 ~ intentions_response - 3,
                                         experiment == 7 ~ intentions_response)) %>%
  group_by(subject) %>%
  summarize(n = n(),
            mean_intentions = round(mean(intentions_response), 2), .groups = "drop") %>%
  mutate(complete_intentions = case_when(n == 3 ~ "complete", 
                                         n > 3 ~ "excess", 
                                         n < 3 ~ "partial")) %>%
  select(-n)
...
data_detection_1 <- data_trial_level %>%
  select(subject, 
         deepfake_detection_open = deepfake_check) %>%
  drop_na()

data_detection_2 <- data_trial_level %>%
  select(subject, 
         deepfake_detection_open = deepfake_video_check) %>%
  drop_na()

data_detection_3 <- data_trial_level %>%
  select(subject, 
         deepfake_detection_open = deepfake_audio_check) %>%
  drop_na()

data_detection_4 <- data_trial_level %>% # exp 7
  select(subject, 
         deepfake_detection_open = deepfake_video_check_2) %>%
  drop_na()


data_awareness_1 <- data_trial_level %>%
  select(subject, 
         deepfake_awareness_open = deepfake_concept_check) %>%
  drop_na()

data_awareness_2 <- data_trial_level %>% # exp 7
  select(subject, 
         deepfake_awareness_open = deepfake_concept_check_2) %>%
  drop_na()


data_detection_and_awareness_open <- 
  bind_rows(data_detection_1,
            data_detection_2,
            data_detection_3,
            data_detection_4) %>%
  full_join(bind_rows(data_awareness_1,
                      data_awareness_2), by = "subject")
...
data_detection_closed <- data_trial_level %>%
  select(subject, 
         deepfake_detection_closed = deepfake_video_check_1) %>%
  drop_na()

data_awareness_closed <- data_trial_level %>%
  select(subject, 
         deepfake_awareness_closed = deepfake_concept_check_1) %>%
  mutate(deepfake_awareness_closed = case_when(deepfake_awareness_closed == "Yes" ~ "aware",
                                               deepfake_awareness_closed == "No" ~ "unaware")) %>%
  drop_na()
...
data_individual_differences_temp <- data_preprocessed %>%  # ideally this would use data_trial_level so as not to return to using the raw data, but wrangling trial_level would need to be reworked. These variables aren't of centrla interest so I haven't done it.
  select(subject, 
         ends_with("_bcti"), 
         starts_with("crt_"), 
         starts_with("ocq_"),
...
master_rei_n_items <- 20
master_me_n_items <- 12
...
ras_min_item <- 1
rei_min_item <- 1
me_awareness_min_item <- 1
me_accuracy_min_item <- 1  
me_sharing_min_item <- 1
...
ras_max_item <- 5
rei_max_item <- 7
me_awareness_max_item <- 3
me_accuracy_max_item <- 4 
me_sharing_max_item <- 3
...
data_me_awareness <- data_individual_differences_temp %>%
  ...
  summarize(me_fake_news_awareness_sum = sum(me_fake_news, na.rm = TRUE),
            ...
            me_n_items = n(),
            .groups = "drop") %>%
  mutate(me_fake_news_awareness_sum = ifelse(me_n_items == master_me_n_items, me_fake_news_awareness_sum, NA),
         me_real_news_awareness_sum = ifelse(me_n_items == master_me_n_items, me_real_news_awareness_sum, NA)) %>%
  select(-me_n_items)
...
data_me_accuracy <- data_individual_differences_temp %>%
  ...
  summarize(me_fake_news_accuracy_sum = sum(as.numeric(me_fake_news), na.rm = TRUE),
            ...
            me_n_items = n(),
            .groups = "drop") %>%
  mutate(me_fake_news_accuracy_sum = ifelse(me_n_items == master_me_n_items, me_fake_news_accuracy_sum, NA),
         me_real_news_accuracy_sum = ifelse(me_n_items == master_me_n_items, me_real_news_accuracy_sum, NA)) %>%
  select(-me_n_items)
...
data_me_sharing <- data_individual_differences_temp %>%
  ...
  summarize(me_fake_news_sharing_sum = sum(me_fake_news, na.rm = TRUE),
            ...
            me_n_items = n(),
            .groups = "drop") %>%
  mutate(me_fake_news_sharing_sum = ifelse(me_n_items == master_me_n_items, me_fake_news_sharing_sum, NA),
         me_real_news_sharing_sum = ifelse(me_n_items == master_me_n_items, me_real_news_sharing_sum, NA)) %>%
  select(-me_n_items)
...
data_individual_differences <- data_aot %>%
  ...
  mutate(aot_actively_openminded_thinking_pomp = pomp(aot_actively_openminded_thinking_sum,
         ...
         rei_experiential_pomp                 = pomp(rei_experiential_sum, 
                                                      ...
                                                      rei_min_item,
                                                      rei_max_item)) %>%
  select(subject, contains("_pomp"), contains("_sum"))
...
data_participant_level <- data_conditions_and_demographics %>% 
  ...
  full_join(data_detection_and_awareness_open,           by = "subject") %>% 
  full_join(data_detection_closed,                       by = "subject") %>% 
  full_join(data_awareness_closed,                       by = "subject") %>%
  full_join(data_induction_linger_time,                  by = "subject") %>%
  full_join(data_induction_description,                  by = "subject") %>% 
  full_join(data_exploratory_questions,                  by = "subject") %>%
  full_join(data_individual_differences,                 by = "subject") %>%
  full_join(data_self_reported_evaluations_item_level,   by = "subject") %>%
  full_join(data_behavioural_intentions_item_level,      by = "subject") %>%
  ...
  mutate(mean_self_reported_evaluation_recoded_for_source_valence = ifelse(source_valence == "negative", 
                                                                              mean_self_reported_evaluation*-1, 
                                                                              mean_self_reported_evaluation),
         IAT_D2_recoded_for_source_valence = ifelse(source_valence == "negative", 
                                                       IAT_D2*-1, 
                                                       IAT_D2),
         mean_intentions_recoded_for_source_valence = ifelse(source_valence == "negative", 
                                                                mean_intentions*-1, 
                                                                mean_intentions)) %>%
  
  # compound exclusions
  mutate(exclude_subject = ifelse(complete_iat == "complete" &
                                    complete_selfreport == "complete" & 
                                    (complete_intentions == "complete" | is.na(complete_intentions)) & # measure not used in all studies, so NA also acceptable
                                    passed_iat_performance == TRUE, FALSE, TRUE),
         exclude_subject = ifelse(is.na(exclude_subject), TRUE, exclude_subject)) %>%
  
  # retain only distinct rows, remove duplications
  distinct() %>%
  ...
  select(subject, 
         ...
         mean_self_reported_evaluation, 
         mean_intentions,
         deepfake_detection_open,
         deepfake_detection_closed,
         deepfake_awareness_open,
         deepfake_awareness_closed,
         
         # intervention
         memory_of_intervention_content,
...
# full data file
write_csv(data_participant_level, "processed/2.2_data_participant_level.csv")

# subset of columns for hand scoring
data_participant_level %>%
  # exclude participants who did not provide responses on either question (or were not delivered the question, in earlier experiments) 
  filter(!is.na(deepfake_detection_open) | !is.na(deepfake_awareness_open)) %>%
  # exclude participants with partial or excess data who met exclusion criteria
  filter(exclude_subject == FALSE) %>%
  select(subject, experiment, deepfake_detection_open, deepfake_awareness_open) %>%
  write_csv("processed/2.3_data_participant_level_for_hand_scoring.csv")

# item level IAT for internal reliability analysis
write_csv(data_iat_item_level, "processed/2.4_data_iat_item_level.csv")
...
```{r}
sean <- data_processed %>%
  select(experiment_condition, source_valence, deepfake_detection_open, deepfake_detection_closed, deepfake_awareness_open, deepfake_awareness_closed)
