...
data_trial_level <- data_preprocessed %>%
  ...
  mutate(task = tolower(task),
         ...
         task = case_when(task == "video_positive_baseline"  ~ "video_positive_genuine",
                          ...
                          TRUE ~ task),
         experiment_condition2 = case_when(task %in% c("video_positive_deepfaked",
  ...
  group_by(subject) %>%
  fill(matches("experiment_condition2"), .direction = "down") %>%
  fill(matches("experiment_condition2"), .direction = "up") %>%
  ungroup() %>%
...
data_completeness_iat <- data_trial_level_iat %>%
  count(subject) %>%
  mutate(complete_iat = case_when(n == 128 ~ "complete", 
                                  n > 128 ~ "excess", 
                                  n < 128 ~"partial")) %>%
  select(-n)
