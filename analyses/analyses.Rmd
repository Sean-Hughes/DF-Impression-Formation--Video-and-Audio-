---
title: "Analyses"
subtitle: "DVs standardized by SD (by experiment and condition)" 
author: "Ian Hussey"
...
NB all DVs were standardised by 1SD, within each experiment and experiment condition. Beta estimtes from models are thereofore standardized betas, and the nature of the standardization makes them somewhat comparable to Cohen's *d*. See Lorah (2018) Effect size measures for multilevel models: definition, interpretation, and TIMSS example. Large-scale Assess Educ 6, 8. https://doi.org/10.1186/s40536-018-0061-2
...
data_after_exclusions <- data_processed %>%
  ...
  # standardize by experiment and condition
  group_by(experiment, experiment_condition) %>%
...
ggplot(data_after_exclusions, aes(mean_self_reported_evaluation, color = as.factor(experiment))) +
  geom_density() +
  facet_wrap( ~ experiment_condition + source_valence) +
  ggtitle("Standardized scores")

ggplot(data_after_exclusions, aes(IAT_D2, color = as.factor(experiment))) +
  geom_density() +
  facet_wrap( ~ experiment_condition + source_valence) +
  ggtitle("Standardized scores")

ggplot(data_after_exclusions, aes(mean_intentions, color = as.factor(experiment))) +
  geom_density() +
  facet_wrap( ~ experiment_condition + source_valence) +
  ggtitle("Standardized scores")
...
fit_selfreport <-
  brm(formula = mean_self_reported_evaluation ~ source_valence * experiment_condition + (1 | experiment),
      ...
      file    = "models/fit_selfreport_std",
...
fit_intentions <-
  brm(formula = mean_intentions ~ source_valence * experiment_condition, # no random effect for experiment as only exp 6 assessed intentions
      data    = data_after_exclusions,
      file    = "models/fit_intentions_std",
      prior   = prior(normal(0, 10)),
      iter    = 6000,
      warmup  = 3000,
      control = list(adapt_delta = 0.99),  # to avoid divergent transitions
...
Interpreation of Kappa (Altman 1999, Landis JR, 1977):
...
fit_selfreport_deepfaked_detected <-
  brm(formula = mean_self_reported_evaluation ~ source_valence + (1 | experiment),
      ...
      file    = "models/fit_selfreport_deepfaked_detected_std",
      prior   = prior(normal(0, 10)),
      iter    = 6000,
      warmup  = 3000,
      control = list(adapt_delta = 0.99),  # to avoid divergent transitions
...
noninferiority_sr_deepfaked_detected <-
  ifelse(estimates_sr %>% 
           filter(Parameter == "effect_genuine") %>% 
           pull(CI_95_lower) <
           estimates_sr_deepfaked_detected %>% 
           filter(Parameter == "effect_deepfaked_detected") %>% 
           pull(CI_90_lower),
         "Deepfakes were found to be non-inferior to genuine content",
         "Deepfakes were not found to be non-inferior to genuine content")
...
`r noninferiority_sr_deepfaked_detected`
...
fit_implicit_deepfaked_detected <-
  brm(formula = IAT_D2 ~ source_valence + (1 | experiment),
      data    = data_detectors_subset,
      file    = "models/fit_implicit_deepfaked_detected_std",
      prior   = prior(normal(0, 10)),
      iter    = 6000,
      warmup  = 3000,
      control = list(adapt_delta = 0.99),  # to avoid divergent transitions
...
noninferiority_imp_deepfaked_detected <-
  ifelse(estimates_imp %>% 
           filter(Parameter == "effect_genuine") %>% 
           pull(CI_95_lower) <
           estimates_imp_deepfaked_detected %>% 
           filter(Parameter == "effect_deepfaked_detected") %>% 
...
`r noninferiority_imp_deepfaked_detected`
...
fit_intentions_deepfaked_detected <-
  brm(formula = mean_intentions ~ source_valence, # no random effect for experiment as only exp 6 assessed intentions
      data    = data_detectors_subset,
      file    = "models/fit_intentions_deepfaked_detected_std",
      prior   = prior(normal(0, 10)),
      iter    = 6000,
      warmup  = 3000,
      control = list(adapt_delta = 0.99),  # to avoid divergent transitions
...
noninferiority_intentions_deepfaked_detected <-
  ifelse(estimates_intentions %>% 
           filter(Parameter == "effect_genuine") %>% 
           pull(CI_95_lower) <
           estimates_intentions_deepfaked_detected %>% 
           filter(Parameter == "effect_deepfaked_detected") %>% 
...
`r noninferiority_intentions_deepfaked_detected`
