...
#plot_model(fit_sr)
plot_model(fit_sr, type = "pred", terms = c("source_valence", "experiment_condition"))

# percent moderation
draws_sr <- 
  bind_cols(
    select(spread_draws(fit_sr, b_source_valencepositive), b_source_valencepositive),
    select(spread_draws(fit_sr, b_experiment_conditiondeepfaked), b_experiment_conditiondeepfaked),
    select(spread_draws(fit_sr, `b_source_valencepositive:experiment_conditiondeepfaked`), `b_source_valencepositive:experiment_conditiondeepfaked`)
  ) %>%
...
#plot_model(fit_imp)
plot_model(fit_imp, type = "pred", terms = c("source_valence", "experiment_condition"))

# percent moderation
draws_imp <- 
  bind_cols(
    select(spread_draws(fit_imp, b_source_valencepositive), b_source_valencepositive),
    select(spread_draws(fit_imp, b_experiment_conditiondeepfaked), b_experiment_conditiondeepfaked),
    select(spread_draws(fit_imp, `b_source_valencepositive:experiment_conditiondeepfaked`), `b_source_valencepositive:experiment_conditiondeepfaked`)
  ) %>%
  rename(main_valence = b_source_valencepositive,
         main_experiment_condition = b_experiment_conditiondeepfaked,
         interaction = `b_source_valencepositive:experiment_conditiondeepfaked`) %>%
...
if(file.exists("models/classification_bootstraps")){
  
  results_bootstraps <- read_rds("results_bootstraps.rds")
...
fit_sr_deepfaked_detected <- data_after_exclusions %>%
  ...
  brm(formula = mean_self_reported_evaluation ~ source_valence + (1 | experiment),
      data    = ., 
      file    = "models/fit_sr_deepfaked_detected",
      prior   = prior(normal(0, 10)),
      iter    = 6000,
      warmup  = 3000,
      control = list(adapt_delta = 0.95),
...
# # inspect convergence
# fit_sr_deepfaked_detected

#plot_model(fit_sr_deepfaked_detected)
plot_model(fit_sr_deepfaked_detected, type = "pred", terms = "source_valence")

draws_sr_deepfaked_detected <- 
  select(spread_draws(fit_sr_deepfaked_detected, b_source_valencepositive), b_source_valencepositive) %>%
...
fit_imp_deepfaked_detected <- 
  data_after_exclusions %>%
  filter(experiment_condition == "deepfaked" & deepfake_detected == TRUE) %>%
  brm(formula = IAT_D2 ~ source_valence + (1 | experiment),
      data    = ., 
      file    = "models/fit_imp_deepfaked_detected",
      prior   = prior(normal(0, 10)),
      iter    = 6000,
      warmup  = 3000,
      control = list(adapt_delta = 0.95),
...
#plot_model(fit_imp_deepfaked_detected)
plot_model(fit_imp_deepfaked_detected, type = "pred", terms = "source_valence")

draws_imp_deepfaked_detected <- 
  select(spread_draws(fit_imp_deepfaked_detected, b_source_valencepositive), b_source_valencepositive) %>%
  rename(effect_deepfake_detected = b_source_valencepositive) 
