---
title: "Confirmatory Analyses: JEP: General"
subtitle: "Preregistered analyses for experiment 5"
...
data_processed <- read.csv("../data/processed/4_data_participant_level_with_hand_scoring.csv") %>%
  ...
  filter(experiment == 7) %>%
  # rename experiment so that it is consistent with what is reported in the manuscript (i.e., Experiment 5)
  mutate(experiment_renamed = case_when(experiment == 7 ~ 5)) %>%
  select(-experiment) %>%
  rename(experiment = experiment_renamed,
         informational_content = source_valence,
         content_type = experiment_condition) %>%
  relocate(experiment, .after = subject) %>%
  arrange(experiment) %>%
  
  
  # set factor levels for t test comparisons
  mutate(informational_content = fct_relevel(informational_content,
                                      "negative",
                                      "positive"),
         content_type = fct_relevel(content_type,
                                            "genuine",
                                            "deepfaked"),
...
data_after_exclusions <- data_processed %>%
  ...
  group_by(experiment, content_type, informational_content) %>%
...
ggplot(data_after_exclusions, aes(mean_self_reported_evaluation)) +
  geom_density() +
  facet_wrap( ~ content_type + informational_content) +
...
# H1: Can online content change attitudes and intentions? 

# H2: Are Deepfakes as effective as genuine online content in changing attitudes and intentions?

- Analyses employ Bayesian linear models with informational_content (positive vs negative information about the target), content_type (authentic vs. deepfaked video) and their interaction as IVs. This could therefore be described as akin to a Bayesian ANOVA.

- DVs were standardize as noted above. Fitted model estimates represent standardized beta values (which due to the specifics of the standardization have comparable [but not exact] interpretation as Cohen's d values).
...
H1 hypotheses were tested using a Bayesian linear model to estimate a 95% Credible Interval on standardized effect size change in attitudes and intentions between informational content (positive vs. negative) conditions. Credible Intervals whose lower bounds were > 0 were considered evidence in support of a given hypothesis.

For H2, if the lower bound of the 95% CI of the authentic video condition is < the lower bound of the 90% CI of the Deepfaked video condition (i.e., the difference between informational content conditions in each subgroups), this as considered evidence in support of the alternative hypothesis (i.e., evidence of non-inferiority in estimated means; that Deepfakes are as good as genuine content in changing attitudes and intentions).
...
fit_confirmatory_selfreport <-
  brm(formula = mean_self_reported_evaluation ~ informational_content * content_type,
      family = gaussian(),
...
plot_model(fit_confirmatory_selfreport, type = "pred", terms = c("informational_content", "content_type"))

# percent moderation
draws_sr <-
  bind_cols(
    select(spread_draws(fit_confirmatory_selfreport, b_informational_contentpositive), b_informational_contentpositive),
    select(spread_draws(fit_confirmatory_selfreport, b_content_typedeepfaked), b_content_typedeepfaked),
    select(spread_draws(fit_confirmatory_selfreport, `b_informational_contentpositive:content_typedeepfaked`), `b_informational_contentpositive:content_typedeepfaked`)
  ) %>%
  rename(main_valence = b_informational_contentpositive,
         main_content_type = b_content_typedeepfaked,
         interaction = `b_informational_contentpositive:content_typedeepfaked`) %>%
  mutate(effect_genuine = main_valence,
         effect_deepfaked = main_valence + interaction,
         #percent_moderation = (main_content_type + interaction)/main_valence *100,  # alt method, same result
...
estimates_sr %>%
  ...
  # rename the variables here so that they are consistent with the naming conventions in the paper
  
  mutate(Parameter = replace(Parameter, Parameter == "main_valence", "main_effect_information_content")) %>%
  mutate(Parameter = replace(Parameter, Parameter == "main_content_type", "main_effect_content_type")) %>% 
...
The informational content of the genuine videos (i.e., positive vs. negative) will influence participants’ self-reported evaluations. 

- Result: `r H1a`

*H1b*

The informational content of the Deepfaked videos (i.e., positive vs. negative) will influence participants’ self-reported evaluations.

- Result: `r H1b`

*H2a*

Change in self-reported evaluations (i.e., between informational content conditions) induced by Deepfaked video content will be non-inferior to genuine content.
...
fit_confirmatory_implicit <-
  brm(formula = IAT_D2 ~ informational_content * content_type,
      family = gaussian(),
      data    = data_after_exclusions,
...
check_prior(fit_confirmatory_implicit) %>%
    ...
    mutate(Parameter = replace(Parameter, Parameter == "b_source_valencepositive", "b_information_content"),
         Parameter = replace(Parameter, Parameter == "b_experiment_conditiondeepfaked", "b_content_type_deepfaked"),
         Parameter = replace(Parameter, Parameter == "b_source_valencepositive:experiment_conditiondeepfaked", "b_information_content:content_type_deepfaked")) %>%
...
plot_model(fit_confirmatory_implicit, type = "pred", terms = c("informational_content", "content_type"))

# percent moderation
draws_imp <-
  bind_cols(
    select(spread_draws(fit_confirmatory_implicit, b_informational_contentpositive), b_informational_contentpositive),
    select(spread_draws(fit_confirmatory_implicit, b_content_typedeepfaked), b_content_typedeepfaked),
    select(spread_draws(fit_confirmatory_implicit, `b_informational_contentpositive:content_typedeepfaked`), `b_informational_contentpositive:content_typedeepfaked`)
  ) %>%
  rename(main_valence = b_informational_contentpositive,
         main_experiment_condition = b_content_typedeepfaked,
         interaction = `b_informational_contentpositive:content_typedeepfaked`) %>%
  mutate(effect_genuine = main_valence,
         effect_deepfaked = main_valence + interaction,
...
estimates_imp %>%
  ...
  mutate(Parameter = replace(Parameter, Parameter == "main_valence", "main_effect_information_content")) %>%
  mutate(Parameter = replace(Parameter, Parameter == "main_experiment_condition", "main_effect_content_type")) %>% 
...
The informational content of the genuine videos (i.e., positive vs. negative) will influence participants’ IAT D2 scores.

- Result: `r H1c`

*H1d*

The informational content of the Deepfaked videos (i.e., positive vs. negative) will influence participants’ IAT D2 scores.

- Result: `r H1d`

*H2b*

Change in IAT D2 scores (i.e., between informational content conditions) induced by Deepfaked video content will be non-inferior to those induced by genuine videos.
...
fit_confirmatory_intentions <-
  brm(formula = mean_intentions ~ informational_content * content_type, 
      family = gaussian(),
      data    = data_after_exclusions,
...
informational_content * content_type
...
plot_model(fit_confirmatory_intentions, type = "pred", terms = c("informational_content", "content_type"))

# percent moderation
draws_intentions <-
  bind_cols(
    select(spread_draws(fit_confirmatory_intentions, b_informational_contentpositive), b_informational_contentpositive),
    select(spread_draws(fit_confirmatory_intentions, b_content_typedeepfaked), b_content_typedeepfaked),
    select(spread_draws(fit_confirmatory_intentions, `b_informational_contentpositive:content_typedeepfaked`), `b_informational_contentpositive:content_typedeepfaked`)
  ) %>%
  rename(main_informational_content = b_informational_contentpositive,
         main_content_type = b_content_typedeepfaked,
         interaction = `b_informational_contentpositive:content_typedeepfaked`) %>%
  mutate(effect_genuine = main_informational_content,
         effect_deepfaked = main_informational_content + interaction,
         #percent_moderation = (main_content_type + interaction)/main_informational_content *100,  # alt method, same result
         percent_comparison = (effect_deepfaked/effect_genuine)*100)
...
The informational content of the genuine videos (i.e., positive vs. negative) will influence participants’ behavioral intention responses. 

- Result: `r H1e`

*H1f*

The informational content of the Deepfaked videos (i.e., positive vs. negative) will influence participants’ behavioral intention responses.

- Result: `r H1f`

*H2c*

Change in behavioral intentions (i.e., between informational content conditions) induced by Deepfaked video content will be non-inferior to genuine content.
...
data_classifications <- data_after_exclusions %>%
  select(content_type, deepfake_detection_closed) %>%
  drop_na()

truth <- factor(data_classifications$content_type,
...
bootstrap_categorization_stats <- function(split) {
  ...
  truth <- factor(analysis(split)$content_type,
                  levels = rev(c("genuine", "deepfaked")))
...
truth_subset <- factor(data_classifications_subset$content_type,
                levels = rev(c("genuine", "deepfaked")))
...
# H4: Are people aware that content can be Deepfaked before they take part in the study, and does this make them better at detecting them?
...
It would of course be possible include data from both content_types and add it to the model, however interpreting the two and three way interactions is less intuitive. Given this question is of secondary importance, we therefore elected for the more simple analysis focusing on awareness and the FNR/TPR.
...
plot(conditional_effects(fit_confirmatory_poisson_awareness_detection, 
                         effects = "detection:awareness"), ask = FALSE)
...
draws_awareness_detection <- posterior_samples(fit_confirmatory_poisson_awareness_detection) %>%
  ...
  mutate(effect_awareness_increases_detection = awarenessTRUE + interaction)
...
# hypothesis testing
H4 <- ifelse((estimates_awareness_detection %>% filter(Parameter == "effect_awareness_increases_detection") %>%
                pull(CI_95_lower)) > 1, 
              "Accepted", "Rejected")
...
comparison_string_awareness_detection <-
  paste0("Individuals who were aware of the concept of Deepfakes prior to participating in the study were ",
         estimates_awareness_detection %>% 
           filter(Parameter == "effect_awareness_increases_detection") %>% 
           pull(incidence_rate_ratio_MAP) %>% 
           round(1), 
...
Using the subset of participants who were in the Deepfake condition, we calculated counts for each of the combinations of the Deepfake concept check and Deepfake detection questions (e.g., awareness = TRUE & detection = TRUE, awareness = TRUE & detection = FALSE, etc.). 

We then use a Bayesian Poisson model to estimate a 95% Credible Interval around the effect’s Incidence Rate Ratio. A Credible Interval whose lower bound is > 1 will be considered evidence in support of this hypothesis. Estimated marginal predicted probabilities will also be reported.
...
Subset who received deepfaked videos and were aware of the concept prior to the experiment. Same Bayesian multilevel models as employed above, using only informational_content as IV, i.e., to detect whether learning effects are credibly non-zero in this subset. 
...
data_aware_subset_n <- data_after_exclusions %>%
  filter(content_type == "deepfaked" & deepfake_awareness_closed == "aware") %>%
  count(deepfake_awareness_closed) %>%
...
fit_confirmatory_selfreport_deepfaked_aware <-
  brm(formula = mean_self_reported_evaluation ~ informational_content,
      family  = gaussian(),
...
plot_model(fit_confirmatory_selfreport_deepfaked_aware, type = "pred", terms = "informational_content")

# results table
draws_sr_deepfaked_aware <-
  select(spread_draws(fit_confirmatory_selfreport_deepfaked_aware, b_informational_contentpositive), b_informational_contentpositive) %>%
  rename(effect_deepfaked_aware = b_informational_contentpositive)
...
fit_confirmatory_implicit_deepfaked_aware <-
  brm(formula = IAT_D2 ~ informational_content,
      family  = gaussian(),
      data    = data_aware_subset,
...
plot_model(fit_confirmatory_implicit_deepfaked_aware, type = "pred", terms = "informational_content")

# results table
draws_imp_deepfaked_aware <-
  select(spread_draws(fit_confirmatory_implicit_deepfaked_aware, b_informational_contentpositive), b_informational_contentpositive) %>%
  rename(effect_deepfaked_aware = b_informational_contentpositive)
...
fit_confirmatory_intentions_deepfaked_aware <-
  brm(formula = mean_intentions ~ informational_content, 
      family  = gaussian(),
      data    = data_aware_subset,
...
plot_model(fit_confirmatory_intentions_deepfaked_aware, type = "pred", terms = "informational_content")

# results table
draws_intentions_deepfaked_aware <-
  select(spread_draws(fit_confirmatory_intentions_deepfaked_aware, b_informational_contentpositive), b_informational_contentpositive) %>%
  rename(effect_deepfaked_aware = b_informational_contentpositive)
...
Subset who received deepfaked videos but also detected them. Same Bayesian multilevel models as employed above, using only informational_content as IV, i.e., to detect whether learning effects are credibly non-zero in this subset. 
...
data_detectors_subset_n <- data_after_exclusions %>%
  filter(content_type == "deepfaked" & deepfake_detection_closed == "deepfaked") %>%
...
plot_model(fit_confirmatory_selfreport_deepfaked_detected, type = "pred", terms = "informational_content")

# results table
draws_sr_deepfaked_detected <-
  select(spread_draws(fit_confirmatory_selfreport_deepfaked_detected, b_informational_contentpositive), b_informational_contentpositive) %>%
  rename(effect_deepfaked_detected = b_informational_contentpositive)
...
plot_model(fit_confirmatory_implicit_deepfaked_detected, type = "pred", terms = "informational_content")

# results table
draws_imp_deepfaked_detected <-
  select(spread_draws(fit_confirmatory_implicit_deepfaked_detected, b_informational_contentpositive), b_informational_contentpositive) %>%
  rename(effect_deepfaked_detected = b_informational_contentpositive)
...
plot_model(fit_confirmatory_intentions_deepfaked_detected, type = "pred", terms = "informational_content")

# results table
draws_intentions_deepfaked_detected <-
  select(spread_draws(fit_confirmatory_intentions_deepfaked_detected, b_informational_contentpositive), b_informational_contentpositive) %>%
  rename(effect_deepfaked_detected = b_informational_contentpositive)
...
Subset who received deepfaked videos, were aware of the concept before the study, and also detected them. Same Bayesian multilevel models as employed above, using only informational_content as IV, i.e., to detect whether learning effects are credibly non-zero in this subset. 
...
plot_model(fit_confirmatory_selfreport_deepfaked_aware_detected, type = "pred", terms = "informational_content")

# results table
draws_sr_deepfaked_aware_detected <-
  select(spread_draws(fit_confirmatory_selfreport_deepfaked_aware_detected, b_informational_contentpositive), b_informational_contentpositive) %>%
  rename(effect_deepfaked_aware_detected = b_informational_contentpositive)
...
plot_model(fit_confirmatory_implicit_deepfaked_aware_detected, type = "pred", terms = "informational_content")

# results table
draws_imp_deepfaked_aware_detected <-
  select(spread_draws(fit_confirmatory_implicit_deepfaked_aware_detected, b_informational_contentpositive), b_informational_contentpositive) %>%
  rename(effect_deepfaked_aware_detected = b_informational_contentpositive)
...
plot_model(fit_confirmatory_intentions_deepfaked_aware_detected, type = "pred", terms = "informational_content")

# results table
draws_intentions_deepfaked_aware_detected <-
  select(spread_draws(fit_confirmatory_intentions_deepfaked_aware_detected, b_informational_contentpositive), b_informational_contentpositive) %>%
  rename(effect_deepfaked_aware_detected = b_informational_contentpositive)
