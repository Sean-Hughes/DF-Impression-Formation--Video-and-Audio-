...
NOTE: During the review process the editor requested that we drop our first two studies on authentic videos and just consider studies with Deepfaked content. In line with this request we have dropped Experiments 1-2 (thus the reason why we have only four studies in our exploratory analyses and reported in the paper itself). Nevertheless, our original analytic strategy was conducted on Experiments 1-6 and thus we report below what was actually carried out by us prior to submitting our paper for review. 
...
fit_exploratory_selfreport_exp46 <-
  data_after_exclusions %>%
  filter(experiment %in% c(4, 6)) %>%
  brm(formula = mean_self_reported_evaluation ~ source_valence * experiment_condition + (1 | experiment),
      family  = gaussian(),
      data    = .,
      file    = "models/fit_exploratory_selfreport_exp46",
      prior   = prior(normal(0, 10)),
      iter    = 10000,
      warmup  = 3000,
      control = list(adapt_delta = 0.99),  # to avoid divergent transitions
...
summary(fit_exploratory_selfreport_exp46)
plot(fit_exploratory_selfreport_exp46, ask = FALSE)
...
check_prior(fit_exploratory_selfreport_exp46) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = FALSE)
...
draws_sr_exp46 <-
  bind_cols(
    select(spread_draws(fit_exploratory_selfreport_exp46, b_source_valencepositive), b_source_valencepositive),
    select(spread_draws(fit_exploratory_selfreport_exp46, b_experiment_conditiondeepfaked), b_experiment_conditiondeepfaked),
    select(spread_draws(fit_exploratory_selfreport_exp46, `b_source_valencepositive:experiment_conditiondeepfaked`), `b_source_valencepositive:experiment_conditiondeepfaked`)
  ) %>%
  rename(main_valence = b_source_valencepositive,
         main_experiment_condition = b_experiment_conditiondeepfaked,
         interaction = `b_source_valencepositive:experiment_conditiondeepfaked`) %>%
...
# results
estimates_sr_exp46 <-
  map_estimate(draws_sr_exp46) %>%
  full_join(bayestestR::hdi(draws_sr_exp46, ci = .95) %>%
              rename(CI_95_lower = CI_low,
                     CI_95_upper = CI_high) %>%
              as_tibble(),
            by = "Parameter") %>%
  full_join(bayestestR::hdi(draws_sr_exp46, ci = .90) %>%
              as_tibble() %>%
              rename(CI_90_lower = CI_low,
                     CI_90_upper = CI_high),
            by = "Parameter") %>%
  full_join(draws_sr_exp46 %>%
              gather(Parameter, value) %>%
              group_by(Parameter) %>%
              summarize(pd = mean(value > 0)) %>%
              mutate(p = pd_to_p(pd, direction = "one-sided")) %>%
...
# hypothesis testing
H1b <- ifelse((estimates_sr_exp46 %>% filter(Parameter == "effect_deepfaked") %>% pull(CI_95_lower)) > 0, 
              "Accepted", "Rejected")
...
fit_exploratory_implicit_exp46 <-
  data_after_exclusions %>%
  filter(experiment %in% c(4, 6)) %>%
  brm(formula = IAT_D2 ~ source_valence * experiment_condition + (1 | experiment),
      family  = gaussian(),
      data    = .,
      file    = "models/fit_exploratory_implicit_exp46",
      prior   = prior(normal(0, 10)),
      iter    = 10000,
      warmup  = 3000,
      control = list(adapt_delta = 0.99),  # to avoid divergent transitions
...
summary(fit_exploratory_implicit_exp46)
plot(fit_exploratory_implicit_exp46, ask = FALSE)
...
check_prior(fit_exploratory_implicit_exp46) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = FALSE)
...
draws_imp_exp46 <-
  bind_cols(
    select(spread_draws(fit_exploratory_implicit_exp46, b_source_valencepositive), b_source_valencepositive),
    select(spread_draws(fit_exploratory_implicit_exp46, b_experiment_conditiondeepfaked), b_experiment_conditiondeepfaked),
    select(spread_draws(fit_exploratory_implicit_exp46, `b_source_valencepositive:experiment_conditiondeepfaked`), `b_source_valencepositive:experiment_conditiondeepfaked`)
  ) %>%
  rename(main_valence = b_source_valencepositive,
         main_experiment_condition = b_experiment_conditiondeepfaked,
         interaction = `b_source_valencepositive:experiment_conditiondeepfaked`) %>%
...
# results table
estimates_imp_exp46 <-
  map_estimate(draws_imp_exp46) %>%
  full_join(bayestestR::hdi(draws_imp_exp46, ci = .95) %>%
              rename(CI_95_lower = CI_low,
                     CI_95_upper = CI_high) %>%
              as_tibble(),
            by = "Parameter") %>%
  full_join(bayestestR::hdi(draws_imp_exp46, ci = .90) %>%
              as_tibble() %>%
              rename(CI_90_lower = CI_low,
                     CI_90_upper = CI_high),
            by = "Parameter") %>%
  full_join(draws_imp_exp46 %>%
              gather(Parameter, value) %>%
              group_by(Parameter) %>%
              summarize(pd = mean(value > 0)) %>%
              mutate(p = pd_to_p(pd, direction = "one-sided")) %>%
...
# hypothesis testing
H1d <- ifelse((estimates_imp_exp46 %>% filter(Parameter == "effect_deepfaked") %>% pull(CI_95_lower)) > 0, 
              "Accepted", "Rejected")
...
fit_exploratory_intentions_exp46 <-
  data_after_exclusions %>%
  filter(experiment %in% c(4, 6)) %>%
  brm(formula = mean_intentions ~ source_valence * experiment_condition, # no random effect for experiment as only exp 6 assessed intentions
      family  = gaussian(),
      data    = .,
      file    = "models/fit_exploratory_intentions_exp46",
      prior   = prior(normal(0, 10)),
      iter    = 10000,
      warmup  = 3000,
      control = list(adapt_delta = 0.99),  # to avoid divergent transitions
...
summary(fit_exploratory_intentions_exp46)
plot(fit_exploratory_intentions_exp46, ask = FALSE)
...
check_prior(fit_exploratory_intentions_exp46) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = FALSE)
...
# percent moderation
draws_intentions_intentions_exp46 <-
  bind_cols(
    select(spread_draws(fit_exploratory_intentions_exp46, b_source_valencepositive), b_source_valencepositive),
    select(spread_draws(fit_exploratory_intentions_exp46, b_experiment_conditiondeepfaked), b_experiment_conditiondeepfaked),
    select(spread_draws(fit_exploratory_intentions_exp46, `b_source_valencepositive:experiment_conditiondeepfaked`), `b_source_valencepositive:experiment_conditiondeepfaked`)
  ) %>%
  rename(main_valence = b_source_valencepositive,
         main_experiment_condition = b_experiment_conditiondeepfaked,
         interaction = `b_source_valencepositive:experiment_conditiondeepfaked`) %>%
...
# results table
estimates_intentions_intentions_exp46 <-
  map_estimate(draws_intentions_intentions_exp46) %>%
  full_join(bayestestR::hdi(draws_intentions_intentions_exp46, ci = .95) %>%
              rename(CI_95_lower = CI_low,
                     CI_95_upper = CI_high) %>%
              as_tibble(),
            by = "Parameter") %>%
  full_join(bayestestR::hdi(draws_intentions_intentions_exp46, ci = .90) %>%
              as_tibble() %>%
              rename(CI_90_lower = CI_low,
                     CI_90_upper = CI_high),
            by = "Parameter") %>%
  full_join(draws_intentions_intentions_exp46 %>%
              gather(Parameter, value) %>%
              group_by(Parameter) %>%
              summarize(pd = mean(value > 0)) %>%
              mutate(p = pd_to_p(pd, direction = "one-sided")) %>%
...
# hypothesis testing
H1f <- ifelse((estimates_intentions_intentions_exp46 %>% filter(Parameter == "effect_deepfaked") %>% pull(CI_95_lower)) > 0, 
              "Accepted", "Rejected")
