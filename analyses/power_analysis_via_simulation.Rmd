...
if(file.exists("models/simulation_iterations_self_report.csv")){
  ...
  simulation_iterations_self_report <- fit_selfreport %>%
    fit_initial_model(fitted_empirical_model = ., filename = "fit_simulation_selfreport", N = N_participants) %>%
    simulation(fit_initial_model_object = ., n_sim = N_simulations)
  # write to disk
  simulation_iterations_self_report %>% write_csv("models/simulation_iterations_self_report.csv")
}
