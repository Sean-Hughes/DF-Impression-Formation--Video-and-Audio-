...
results_correlations_sr %>%
  mutate(variables = paste0("SR - ", colnames(select(data_after_exclusions, ends_with("_pomp"))))) %>%
...
results_correlations_imp %>%
  mutate(variables = paste0("Implicit - ", colnames(select(data_after_exclusions, ends_with("_pomp"))))) %>%
  select(variables, r = raw.r, ci_lower = raw.lower, ci_upper = raw.upper, p = raw.p) %>%
  arrange(p) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
...
results_correlations_int %>%
  mutate(variables = paste0("Intentions - ", colnames(select(data_after_exclusions, ends_with("_pomp"))))) %>%
  select(variables, r = raw.r, ci_lower = raw.lower, ci_upper = raw.upper, p = raw.p) %>%
  arrange(p) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
...
SHOULD THIS ONLY BE THE SUBSET OF DEEPFAKE CONDITION PEOPLE? OR THE SUBSET OF CORRECT CLASSIFIERS? OR?
