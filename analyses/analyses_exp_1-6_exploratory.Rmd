...
fit_exploratory_classification_bootstraps %>%
  group_by(variable) %>%
  ...
  select(variable, observed, median, ci_lower, ci_upper) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                full_width = FALSE)
...
- Results will report the observed estimate and the bootstrapped 95% CIs. Bootstrapped median estimate is reported here as a robustness test. 
...
fit_exploratory_classification_bootstraps_subset %>%
  group_by(variable) %>%
  summarize(median   = quantile(value, 0.500),
            ci_lower = quantile(value, 0.025),
            ci_upper = quantile(value, 0.975),
...
plot_model(fit_exploratory_poisson_awareness_detection)
