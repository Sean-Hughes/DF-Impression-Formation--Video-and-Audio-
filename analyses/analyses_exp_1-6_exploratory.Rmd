...
data_after_exclusions %>%
  dplyr::select(deepfake_concept_check) %>%
  drop_na() %>%
...
data_counts_awareness_detection <- data_after_exclusions %>%
  ...
  dplyr::select(experiment, deepfake_concept_check, deepfake_detected) %>%
  drop_na() %>%
  count(experiment, deepfake_concept_check, deepfake_detected) %>%
  group_by(experiment) %>%
  mutate(counts = n,
         awareness = as.factor(deepfake_concept_check),
...
# RQ5: Does knowing content is Deepfaked make you immune to its influence?
...
# hypothesis testing
H5a <- ifelse((estimates_sr_deepfaked_detected %>% filter(Parameter == "effect_deepfaked_detected") %>%
                 pull(CI_95_lower)) > 0, 
              "Accepted", "Rejected")
...
# hypothesis testing
H5b <- ifelse((estimates_imp_deepfaked_detected %>% filter(Parameter == "effect_deepfaked_detected") %>%
                 pull(CI_95_lower)) > 0, 
              "Accepted", "Rejected")
...
# hypothesis testing
H5c <- ifelse((estimates_intentions_deepfaked_detected %>% filter(Parameter == "effect_deepfaked_detected") %>%
                 pull(CI_95_lower)) > 0, 
              "Accepted", "Rejected")
