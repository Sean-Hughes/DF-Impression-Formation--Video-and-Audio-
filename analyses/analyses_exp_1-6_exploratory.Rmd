...
data_processed <- read.csv("../data/processed/4_data_participant_level_with_hand_scoring.csv") %>%
  ...
  filter(experiment %in% c(1, 2, 3, 4, 5, 6, 7)) %>%
...
# RQ1 & 2: Can online audio/video content establish first impressions, even when it’s not real?; Are Deepfakes just as good as genuine online content at establishing first impressions?
...
data_after_exclusions %>%
  ...
  mutate(counts = n,
         ...
         percent = round(counts/sum(counts)*100, 1)) %>%
...
pp_check(fit_poisson_awareness_detection, nsamples = 100)
summary(fit_poisson_awareness_detection)
plot(fit_poisson_awareness_detection, ask = FALSE)
...
check_prior(fit_poisson_awareness_detection) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = FALSE)
...
plot_model(fit_poisson_awareness_detection)
# plot(conditional_effects(fit_poisson_awareness_detection), ask = FALSE)
...
draws_awareness_detection <- posterior_samples(fit_poisson_awareness_detection) %>%
...
Participants who reporting being aware of the concept of Deepfakes prior to participating in the experiment will be better at detectin Deepfakes when presented with one. Specifically, we will use participants in the Deepfake condition to calculate cell counts for the Deepfake concept check and Deepfake detection questions (i.e., what number of participants were observed for each combination of those variables). We will then use a Bayesian Poisson model (model 5) to estimate a 95% Confidence Interval on the Incidence Rate Ratio for the interaction effect between Deepfakes concept awareness and deepfake detection. A Confidence Interval whose lower bound is > 1 will be considered evidence in support of this hypothesis.
...
posterior_predictions_awareness_detection <-
  ...
  add_predicted_draws(model = fit_poisson_awareness_detection, re_formula = NULL) %>%
...
# RQ5: Does knowing about the concept of Deepfaking make you immune to its influence?
...
summary(fit_selfreport_deepfaked_aware)
plot(fit_selfreport_deepfaked_aware, ask = FALSE)
...
check_prior(fit_selfreport_deepfaked_aware) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = FALSE)
...
#plot_model(fit_selfreport_deepfaked_aware)
plot_model(fit_selfreport_deepfaked_aware, type = "pred", terms = "source_valence")

# results table
draws_sr_deepfaked_aware <-
  select(spread_draws(fit_selfreport_deepfaked_aware, b_source_valencepositive), b_source_valencepositive) %>%
...
In the subset of participants who were shown a Deepfaked video and accurately detected that the video was Deepfaked, the content of the Deepfaked videos (i.e., Source Valence) will influence participants’ self-reported evaluations. Specifically, we will use a Bayesian linear model (model 3) to estimate a 95% Confidence Interval on standardized effect size change in self-reported evaluations between Source Valence conditions in the genuine video condition subgroup. Confidence Intervals whose lower bounds are > 0 will be considered evidence in support of this hypothesis.  ######### needs editing #########
...
fit_implicit_deepfaked_aware <-
  brm(formula = IAT_D2 ~ source_valence + (1 | experiment),
      family  = gaussian(),
      data    = data_aware_subset,
      file    = "models/fit_implicit_deepfaked_aware",
      prior   = prior(normal(0, 10)),
      iter    = 10000,
      warmup  = 3000,
      control = list(adapt_delta = 0.99),  # to avoid divergent transitions
...
summary(fit_implicit_deepfaked_aware)
plot(fit_implicit_deepfaked_aware, ask = FALSE)
...
check_prior(fit_implicit_deepfaked_aware) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = FALSE)
...
#plot_model(fit_implicit_deepfaked_aware)
plot_model(fit_implicit_deepfaked_aware, type = "pred", terms = "source_valence")

# results table
draws_imp_deepfaked_aware <-
  select(spread_draws(fit_implicit_deepfaked_aware, b_source_valencepositive), b_source_valencepositive) %>%
  rename(effect_deepfaked_aware = b_source_valencepositive)
...
In the subset of participants who were shown a Deepfaked video and accurately detected that the video was Deepfaked, the content of the Deepfaked videos (i.e., Source Valence) will influence participants’ IAT D2 scores. Specifically, we will use a Bayesian linear model (model 4) to estimate a 95% Confidence Interval on standardized effect size change in IAT D2 scores between Source Valence conditions in the Deepfaked video condition subgroup. Confidence Intervals whose lower bounds are > 0 will be considered evidence in support of this hypothesis.  ######### needs editing #########
...
summary(fit_intentions_deepfaked_aware)
plot(fit_intentions_deepfaked_aware, ask = FALSE)
...
check_prior(fit_intentions_deepfaked_aware) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = FALSE)
...
#plot_model(fit_intentions_deepfaked_aware)
plot_model(fit_intentions_deepfaked_aware, type = "pred", terms = "source_valence")

# results table
draws_intentions_deepfaked_aware <-
  select(spread_draws(fit_intentions_deepfaked_aware, b_source_valencepositive), b_source_valencepositive) %>%
  rename(effect_deepfaked_aware = b_source_valencepositive)
...
# RQ6: Does knowing content is Deepfaked make you immune to its influence?
...
# RQ7: Does both being aware of the concept of Deepfaking and correcting diagnosing that content is Deepfaked make you immune to its influence?
...
data_aware_detectors_subset_n <- data_after_exclusions %>%
  filter(experiment_condition == "deepfaked" & !is.na(deepfake_detection_open_recoded) & !is.na(deepfake_awareness_open_recoded)) %>%
...
fit_selfreport_deepfaked_aware_detected <-
  brm(formula = mean_self_reported_evaluation ~ source_valence + (1 | experiment),
      family  = gaussian(),
      data    = data_aware_detectors_subset,
      file    = "models/fit_selfreport_deepfaked_aware_detected",
      prior   = prior(normal(0, 10)),
      iter    = 10000,
      warmup  = 3000,
      control = list(adapt_delta = 0.99),  # to avoid divergent transitions
...
summary(fit_selfreport_deepfaked_aware_detected)
plot(fit_selfreport_deepfaked_aware_detected, ask = FALSE)
...
check_prior(fit_selfreport_deepfaked_aware_detected) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = FALSE)
...
#plot_model(fit_selfreport_deepfaked_aware_detected)
plot_model(fit_selfreport_deepfaked_aware_detected, type = "pred", terms = "source_valence")

# results table
draws_sr_deepfaked_aware_detected <-
  select(spread_draws(fit_selfreport_deepfaked_aware_detected, b_source_valencepositive), b_source_valencepositive) %>%
...
fit_implicit_deepfaked_aware_detected <-
  brm(formula = IAT_D2 ~ source_valence + (1 | experiment),
      family  = gaussian(),
      data    = data_aware_detectors_subset,
      file    = "models/fit_implicit_deepfaked_aware_detected",
      prior   = prior(normal(0, 10)),
      iter    = 10000,
      warmup  = 3000,
      control = list(adapt_delta = 0.99),  # to avoid divergent transitions
...
summary(fit_implicit_deepfaked_aware_detected)
plot(fit_implicit_deepfaked_aware_detected, ask = FALSE)
...
check_prior(fit_implicit_deepfaked_aware_detected) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = FALSE)
...
#plot_model(fit_implicit_deepfaked_aware_detected)
plot_model(fit_implicit_deepfaked_aware_detected, type = "pred", terms = "source_valence")

# results table
draws_imp_deepfaked_aware_detected <-
  select(spread_draws(fit_implicit_deepfaked_aware_detected, b_source_valencepositive), b_source_valencepositive) %>%
  rename(effect_deepfaked_aware_detected = b_source_valencepositive)
...
fit_intentions_deepfaked_aware_detected <-
  brm(formula = mean_intentions ~ source_valence, # no random effect for experiment as only exp 6 assessed intentions
      family  = gaussian(),
      data    = data_aware_detectors_subset,
      file    = "models/fit_intentions_deepfaked_aware_detected",
      prior   = prior(normal(0, 10)),
      iter    = 10000,
      warmup  = 3000,
      control = list(adapt_delta = 0.99),  # to avoid divergent transitions
...
summary(fit_intentions_deepfaked_aware_detected)
plot(fit_intentions_deepfaked_aware_detected, ask = FALSE)
...
check_prior(fit_intentions_deepfaked_aware_detected) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = FALSE)
...
#plot_model(fit_intentions_deepfaked_aware_detected)
plot_model(fit_intentions_deepfaked_aware_detected, type = "pred", terms = "source_valence")

# results table
draws_intentions_deepfaked_aware_detected <-
  select(spread_draws(fit_intentions_deepfaked_aware_detected, b_source_valencepositive), b_source_valencepositive) %>%
  rename(effect_deepfaked_aware_detected = b_source_valencepositive)
