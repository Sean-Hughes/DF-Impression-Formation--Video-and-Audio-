...
Note that the random effect for experiment was removed from the model as this was causing poor convergence issues. 
...
fit_exploratory_poisson_awareness_detection <- 
  brm(formula = counts ~ 1 + awareness * detection,  # random intercept for experiment dropped to improve model convergence: + (1 | experiment)
      family  = poisson(),
...
plot(conditional_effects(fit_exploratory_poisson_awareness_detection, 
...
For the sake of conveying points in the summary manuscript, e.g., that deepfakes or genuine content were shown to establish attitudes in a specific experiment, we add models and results using these specific subsets. These models were added after the confirmatory experiment 7 was run, and were chosen over adjusting the above models to (for example) include random intercept for experiment or intervention medium.

## Evaluative learning effects in deepfaked group from Experiment 3
...
summary(fit_exploratory_selfreport_exp3)
plot(fit_exploratory_selfreport_exp3, ask = FALSE)
...
check_prior(fit_exploratory_selfreport_exp3) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = FALSE)
...
# plot_model(fit_exploratory_selfreport_exp3)
plot_model(fit_exploratory_selfreport_exp3, type = "pred", terms = c("source_valence", "experiment_condition"))

# percent moderation
draws_sr_exp3 <-
  bind_cols(
    select(spread_draws(fit_exploratory_selfreport_exp3, b_source_valencepositive), b_source_valencepositive),
    select(spread_draws(fit_exploratory_selfreport_exp3, b_experiment_conditiondeepfaked), b_experiment_conditiondeepfaked),
    select(spread_draws(fit_exploratory_selfreport_exp3, `b_source_valencepositive:experiment_conditiondeepfaked`), `b_source_valencepositive:experiment_conditiondeepfaked`)
  ) %>%
  rename(main_valence = b_source_valencepositive,
         main_experiment_condition = b_experiment_conditiondeepfaked,
         interaction = `b_source_valencepositive:experiment_conditiondeepfaked`) %>%
  mutate(effect_deepfaked = main_valence + interaction)

# results
estimates_sr_exp3 <-
  map_estimate(draws_sr_exp3) %>%
  full_join(bayestestR::hdi(draws_sr_exp3, ci = .95) %>%
              rename(CI_95_lower = CI_low,
                     CI_95_upper = CI_high) %>%
              as_tibble(),
            by = "Parameter") %>%
  full_join(bayestestR::hdi(draws_sr_exp3, ci = .90) %>%
              as_tibble() %>%
              rename(CI_90_lower = CI_low,
                     CI_90_upper = CI_high),
            by = "Parameter") %>%
  full_join(draws_sr_exp3 %>%
              select(-percent_comparison) %>%
              gather(Parameter, value) %>%
              group_by(Parameter) %>%
              summarize(pd = mean(value > 0)) %>%
...
# hypothesis testing
H1b <- ifelse((estimates_sr_exp3 %>% filter(Parameter == "effect_deepfaked") %>% pull(CI_95_lower)) > 0, 
              "Accepted", "Rejected")
...
fit_exploratory_implicit_exp3 <-
  data_after_exclusions %>%
  filter(experiment == 3) %>%
  brm(formula = IAT_D2 ~ source_valence * experiment_condition,
      family  = gaussian(),
      data    = .,
      file    = "models/fit_exploratory_implicit_exp3",
      prior   = prior(normal(0, 10)),
      iter    = 10000,
      warmup  = 3000,
      control = list(adapt_delta = 0.99),  # to avoid divergent transitions
...
summary(fit_exploratory_implicit_exp3)
plot(fit_exploratory_implicit_exp3, ask = FALSE)
...
check_prior(fit_exploratory_implicit_exp3) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = FALSE)
...
#plot_model(fit_exploratory_implicit_exp3)
plot_model(fit_exploratory_implicit_exp3, type = "pred", terms = c("source_valence", "experiment_condition"))

# percent moderation
draws_imp_exp3 <-
  bind_cols(
    select(spread_draws(fit_exploratory_implicit_exp3, b_source_valencepositive), b_source_valencepositive),
    select(spread_draws(fit_exploratory_implicit_exp3, b_experiment_conditiondeepfaked), b_experiment_conditiondeepfaked),
    select(spread_draws(fit_exploratory_implicit_exp3, `b_source_valencepositive:experiment_conditiondeepfaked`), `b_source_valencepositive:experiment_conditiondeepfaked`)
  ) %>%
  rename(main_valence = b_source_valencepositive,
         main_experiment_condition = b_experiment_conditiondeepfaked,
         interaction = `b_source_valencepositive:experiment_conditiondeepfaked`) %>%
...
# results table
estimates_imp_exp3 <-
  map_estimate(draws_imp_exp3) %>%
  full_join(bayestestR::hdi(draws_imp_exp3, ci = .95) %>%
              rename(CI_95_lower = CI_low,
                     CI_95_upper = CI_high) %>%
              as_tibble(),
            by = "Parameter") %>%
  full_join(bayestestR::hdi(draws_imp_exp3, ci = .90) %>%
              as_tibble() %>%
              rename(CI_90_lower = CI_low,
                     CI_90_upper = CI_high),
            by = "Parameter") %>%
  full_join(draws_imp_exp3 %>%
              select(-percent_comparison) %>%
              gather(Parameter, value) %>%
              group_by(Parameter) %>%
              summarize(pd = mean(value > 0)) %>%
...
# hypothesis testing
H1d <- ifelse((estimates_imp_exp3 %>% filter(Parameter == "effect_deepfaked") %>% pull(CI_95_lower)) > 0, 
              "Accepted", "Rejected")
...
## Evaluative learning effects in deepfaked group from Experiment 4 and 6
...
fit_exploratory_selfreport_exp4 <-
  data_after_exclusions %>%
  filter(experiment %in% c(4, 6)) %>%
  brm(formula = mean_self_reported_evaluation ~ source_valence * experiment_condition + (1 | experiment),
      family  = gaussian(),
      data    = .,
      file    = "models/fit_exploratory_selfreport_exp4",
      prior   = prior(normal(0, 10)),
      iter    = 10000,
      warmup  = 3000,
      control = list(adapt_delta = 0.99),  # to avoid divergent transitions
...
summary(fit_exploratory_selfreport_exp4)
plot(fit_exploratory_selfreport_exp4, ask = FALSE)
...
check_prior(fit_exploratory_selfreport_exp4) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = FALSE)
...
# plot_model(fit_exploratory_selfreport_exp4)
plot_model(fit_exploratory_selfreport_exp4, type = "pred", terms = c("source_valence", "experiment_condition"))

# percent moderation
draws_sr_exp4 <-
  bind_cols(
    select(spread_draws(fit_exploratory_selfreport_exp4, b_source_valencepositive), b_source_valencepositive),
    select(spread_draws(fit_exploratory_selfreport_exp4, b_experiment_conditiondeepfaked), b_experiment_conditiondeepfaked),
    select(spread_draws(fit_exploratory_selfreport_exp4, `b_source_valencepositive:experiment_conditiondeepfaked`), `b_source_valencepositive:experiment_conditiondeepfaked`)
  ) %>%
  rename(main_valence = b_source_valencepositive,
         main_experiment_condition = b_experiment_conditiondeepfaked,
         interaction = `b_source_valencepositive:experiment_conditiondeepfaked`) %>%
...
# results
estimates_sr_exp4 <-
  map_estimate(draws_sr_exp4) %>%
  full_join(bayestestR::hdi(draws_sr_exp4, ci = .95) %>%
              rename(CI_95_lower = CI_low,
                     CI_95_upper = CI_high) %>%
              as_tibble(),
            by = "Parameter") %>%
  full_join(bayestestR::hdi(draws_sr_exp4, ci = .90) %>%
              as_tibble() %>%
              rename(CI_90_lower = CI_low,
                     CI_90_upper = CI_high),
            by = "Parameter") %>%
  full_join(draws_sr_exp4 %>%
              select(-percent_comparison) %>%
              gather(Parameter, value) %>%
              group_by(Parameter) %>%
              summarize(pd = mean(value > 0)) %>%
...
# hypothesis testing
H1b <- ifelse((estimates_sr_exp4 %>% filter(Parameter == "effect_deepfaked") %>% pull(CI_95_lower)) > 0, 
              "Accepted", "Rejected")
...
fit_exploratory_implicit_exp4 <-
  data_after_exclusions %>%
  filter(experiment %in% c(4, 6)) %>%
  brm(formula = IAT_D2 ~ source_valence * experiment_condition + (1 | experiment),
      family  = gaussian(),
      data    = .,
      file    = "models/fit_exploratory_implicit_exp4",
      prior   = prior(normal(0, 10)),
      iter    = 10000,
      warmup  = 3000,
      control = list(adapt_delta = 0.99),  # to avoid divergent transitions
...
summary(fit_exploratory_implicit_exp4)
plot(fit_exploratory_implicit_exp4, ask = FALSE)
...
check_prior(fit_exploratory_implicit_exp4) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = FALSE)
...
#plot_model(fit_exploratory_implicit_exp4)
plot_model(fit_exploratory_implicit_exp4, type = "pred", terms = c("source_valence", "experiment_condition"))

# percent moderation
draws_imp_exp4 <-
  bind_cols(
    select(spread_draws(fit_exploratory_implicit_exp4, b_source_valencepositive), b_source_valencepositive),
    select(spread_draws(fit_exploratory_implicit_exp4, b_experiment_conditiondeepfaked), b_experiment_conditiondeepfaked),
    select(spread_draws(fit_exploratory_implicit_exp4, `b_source_valencepositive:experiment_conditiondeepfaked`), `b_source_valencepositive:experiment_conditiondeepfaked`)
  ) %>%
  rename(main_valence = b_source_valencepositive,
         main_experiment_condition = b_experiment_conditiondeepfaked,
         interaction = `b_source_valencepositive:experiment_conditiondeepfaked`) %>%
...
# results table
estimates_imp_exp4 <-
  map_estimate(draws_imp_exp4) %>%
  full_join(bayestestR::hdi(draws_imp_exp4, ci = .95) %>%
              rename(CI_95_lower = CI_low,
                     CI_95_upper = CI_high) %>%
              as_tibble(),
            by = "Parameter") %>%
  full_join(bayestestR::hdi(draws_imp_exp4, ci = .90) %>%
              as_tibble() %>%
              rename(CI_90_lower = CI_low,
                     CI_90_upper = CI_high),
            by = "Parameter") %>%
  full_join(draws_imp_exp4 %>%
              select(-percent_comparison) %>%
              gather(Parameter, value) %>%
              group_by(Parameter) %>%
              summarize(pd = mean(value > 0)) %>%
...
# hypothesis testing
H1d <- ifelse((estimates_imp_exp4 %>% filter(Parameter == "effect_deepfaked") %>% pull(CI_95_lower)) > 0, 
              "Accepted", "Rejected")
...
fit_exploratory_intentions_exp4 <-
  data_after_exclusions %>%
  filter(experiment %in% c(4, 6)) %>%
  brm(formula = mean_intentions ~ source_valence * experiment_condition, # no random effect for experiment as only exp 6 assessed intentions
      family  = gaussian(),
      data    = .,
      file    = "models/fit_exploratory_intentions_exp4",
      prior   = prior(normal(0, 10)),
      iter    = 10000,
      warmup  = 3000,
      control = list(adapt_delta = 0.99),  # to avoid divergent transitions
...
summary(fit_exploratory_intentions_exp4)
plot(fit_exploratory_intentions_exp4, ask = FALSE)
...
check_prior(fit_exploratory_intentions_exp4) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = FALSE)
...
#plot_model(fit_exploratory_intentions_exp4)
plot_model(fit_exploratory_intentions_exp4, type = "pred", terms = c("source_valence", "experiment_condition"))

# percent moderation
draws_intentions_intentions_exp4 <-
  bind_cols(
    select(spread_draws(fit_exploratory_intentions_exp4, b_source_valencepositive), b_source_valencepositive),
    select(spread_draws(fit_exploratory_intentions_exp4, b_experiment_conditiondeepfaked), b_experiment_conditiondeepfaked),
    select(spread_draws(fit_exploratory_intentions_exp4, `b_source_valencepositive:experiment_conditiondeepfaked`), `b_source_valencepositive:experiment_conditiondeepfaked`)
  ) %>%
  rename(main_valence = b_source_valencepositive,
         main_experiment_condition = b_experiment_conditiondeepfaked,
         interaction = `b_source_valencepositive:experiment_conditiondeepfaked`) %>%
...
# results table
estimates_intentions_intentions_exp4 <-
  map_estimate(draws_intentions_intentions_exp4) %>%
  full_join(bayestestR::hdi(draws_intentions_intentions_exp4, ci = .95) %>%
              rename(CI_95_lower = CI_low,
                     CI_95_upper = CI_high) %>%
              as_tibble(),
            by = "Parameter") %>%
  full_join(bayestestR::hdi(draws_intentions_intentions_exp4, ci = .90) %>%
              as_tibble() %>%
              rename(CI_90_lower = CI_low,
                     CI_90_upper = CI_high),
            by = "Parameter") %>%
  full_join(draws_intentions_intentions_exp4 %>%
              select(-percent_comparison) %>%
              gather(Parameter, value) %>%
              group_by(Parameter) %>%
              summarize(pd = mean(value > 0)) %>%
...
# hypothesis testing
H1f <- ifelse((estimates_intentions_intentions_exp4 %>% filter(Parameter == "effect_deepfaked") %>% pull(CI_95_lower)) > 0, 
              "Accepted", "Rejected")
